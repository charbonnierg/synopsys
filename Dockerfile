...
RUN python3 -m pip install -U pip setuptools wheel \
    ...
    && python3 -m virtualenv /opt/synopsys --download \
    && mkdir -p /build
...
RUN --mount=type=secret,id=pip-config \
    ...
    && /opt/synopsys/bin/python -m pip install -r /build/requirements.txt
...
RUN /opt/synopsys/bin/python -m pip install /build

FROM $BASE_IMAGE

ARG USER_ID="1000"
ARG GROUP_ID="1000"
ARG USER_NAME="synopsys"

ENV PROJECT_NAME="synopsys"
...
COPY --from=build /opt/synopsys /opt/synopsys

ENV PATH="/opt/synopsys/bin:$PATH"
